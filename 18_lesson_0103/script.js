...
function create_table(list) {
    ...
    for (let i = 0; i < list.length; i++) {
        ...
        for (let key in list[i]) {  // для каждого ключа в объекте
            ...
            if (key === 'annual_debt_increase_pct') {  // если мы нашли ключ с увеличением долга
                td.innerHTML = list[i][key] * 100 + ' %';  // умножить число на 100
            } else {
                td.innerHTML = list[i][key];  // записываю информацию из объекта в ячейку
            }

            tr.appendChild(td);  // добавить ячейку в строку
        }

        table.appendChild(tr); // добавляю строчку к таблице
    }
}
